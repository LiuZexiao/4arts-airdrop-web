...
import { SampleJetton, storeMint, storeMaxSupply, storeReplace, storeAppend, RewardArray, storeUpdateAddressInfo } from "../output/SampleJetton_SampleJetton";
...
(async () => {
    ...
    // 钱包2 信息
    const mnemonics2 = "word word word .....";
    ...
    // *增加最大供应量消息
    let MaxSupply = toNano(100000);
    let packed_MaxSupply = beginCell()
        .store(
            storeMaxSupply({
                $$type: "MaxSupply",  // 消息名称
                max_supply: MaxSupply, //要增加的供应量
            })
        )
        .endCell();
    ...
    let packed_deleteRewardMint = beginCell()
        .storeUint(0, 32)
        .storeStringTail("deleteRewardMint")
        .endCell();
